...
import getPlaybackInfo from './getPlaybackInfo';
// import getTrackManifest from './getTrackManifest';
...
export default class Download {
    ...
    async getSegments() {
        ...
        if (this.details.isTrack) {
            ...
            const segmentManifest = this.videoQuality ?
                segmentManifests.reduce((closest: any, curr: any) => {
                ...
                segmentManifests.reduce((highest: any, curr: any) => {
                    const height = parseInt(curr.resolution.split('x')[1], 10);
    ...
    async getMetadata() {
        ...
        const customMetadata = this.customMetadata?.map((i: any) => ([i[0], formatString(i[1], this.details)])) || [];
        const creditMetadata = [...trackCredits, ...albumCredits].map(credit => credit.tagName ? [credit.tagName, normalizeTag(credit.contributors.map((i: any) => i.name), this.roleTagSeparator)] : null).filter(i => i);

        this.metadata = [
            ['title', this.details.title],
            ['artist', normalizeTag(this.details.artists?.map((i: any) => i.name), !this.useArtistsTag ? this.artistTagSeparator : null)],
            ['artists', this.useArtistsTag ? normalizeTag(this.details.artists?.map((i: any) => i.name), this.artistTagSeparator) : null],
            ['version', track?.version],
            ['album', album?.title],
            ['albumartist', normalizeTag(this.details.albumArtists?.map((i: any) => i.name), !this.useArtistsTag ? this.artistTagSeparator : null)],
            ['albumartists', this.useArtistsTag ? normalizeTag(this.details.albumArtists?.map((i: any) => i.name), this.artistTagSeparator) : null],
    ...
    log(msg: string, level?: string) {
        ...
        if (level) {
            this.logger.log(log, level, true, true);
        } else {
            this.logger.log(log, 'info', true);
        }
    }
}